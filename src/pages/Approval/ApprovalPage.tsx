import { Button, DraggableModal, InvoiceFilter, InvoiceWidget, PageHeading } from '@components';
...
export const ApprovalPage: React.FC = () => {
  ...
  const { selectedApprovalInvoice, setSelectedApprovalInvoice, getApprovalInvoices } = useApproval()
  const [isFilterModalOpen, setIsFilterModalOpen] = useState<boolean>(false);
  const [isApprovalModalOpen, setIsApprovalModalOpen] = useState<boolean>(false);
  const [filters, setFilters] = useState<FilterTypes[]>([
    { id: 1, field: '', condition: '', value: '' },
  ]);
  const { data: approvalInvoices, isLoading: isApprovalInvoicesLoading } = useQuery({
    queryKey: ["approvalInvoices"],
    queryFn: () => getApprovalInvoices()
  })
  const [filteredInvoices, setFilteredInvoices] = useState<Invoice[]>(approvalInvoices?.invoices || []);
  ...
  const formattedInvoices = filteredInvoices?.map((invoice: Invoice) => ({
    ...invoice,
    invoiceDate:
  ...
  return (
    <section className="md:py-9 pt-20 w-screen md:max-w-[calc(100vw-256px)] h-[100dvh] overflow-y-auto">
      ...
      <div className="flex mt-5 justify-between items-center md:px-14 px-2">
        ...
        <DraggableModal
          okText="Add"
          handleOk={() => setFilteredInvoices(handleInvoiceFilters(approvalInvoices?.invoices, filters) || [])}
      ...
      />
      <ApprovalModal handleClose={handleCloseApprovalModal} isModalOpen={isApprovalModalOpen} />
    </section>
  );
};
