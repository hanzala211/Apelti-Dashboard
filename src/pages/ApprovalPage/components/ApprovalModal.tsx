...
export const ApprovalModal: React.FC<{ isModalOpen: boolean, handleClose: () => void }> = ({ isModalOpen, handleClose }) => {
  const { selectedApprovalInvoice, changeStatus, setSelectedApprovalInvoice } = useApproval()
  const queryClient = useQueryClient()
  const approvalInvoices = queryClient.getQueryData<GetApprovalTypes>(['approvalInvoices'])
  const selectedInvoice = approvalInvoices?.invoices[selectedApprovalInvoice || 0]
  ...
  const handleSuccess = () => {
    ...
    toast.success("Operation Successful", "The status of the invoice has been successfully changed.")
  }

  const handleError = () => {
    handleClose()
    setSelectedApprovalInvoice(null)
    toast.error("Error", "An error occurred while changing the status of the invoice.")
  }
  ...
  >
    <header className="h-[7rem] px-4 shadow-lg flex justify-between items-center w-full bg-basicWhite">
      ...
      <div className="flex gap-4 items-center">
        <button
          onClick={() => changeStatusApprovedMutation.mutate({ status: "approved" })}
          disabled={changeStatusRejectedMutation.isPending || changeStatusApprovedMutation.isPending}
          className={`text-basicWhite md:text-[16px] text-[14px] bg-primaryColor px-7 hover:bg-opacity-70 transition-all border-primaryColor border-[1px] duration-200 py-1.5 rounded-full ${changeStatusRejectedMutation.isPending || changeStatusApprovedMutation.isPending && 'bg-blue-700 cursor-not-allowed'}`}
        >
        ...
        <button
          onClick={() => changeStatusRejectedMutation.mutate({ status: "rejected" })}
          disabled={changeStatusRejectedMutation.isPending || changeStatusApprovedMutation.isPending}
          className={`text-basicWhite md:text-[16px] text-[14px] bg-basicRed px-7 hover:bg-opacity-70 transition-all border-basicRed border-[1px] duration-200 py-1.5 rounded-full ${changeStatusRejectedMutation.isPending || changeStatusApprovedMutation.isPending && 'bg-red-700 cursor-not-allowed'}`}
        >
    ...
    <div className="grid md:grid-cols-2 mt-[1px] bg-basicWhite h-full">
      ...
      {selectedInvoice?.fileUrl &&
        <div className="w-full h-[89dvh] md:block hidden border-r-[2px]">
          <img
